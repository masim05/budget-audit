...
describe('unsupported CSV blockers', () => {
  it('rejects unsupported headers and missing filename currency markers', async () => {
    ...
    await expect(new CsvStatementSource(folder).load()).rejects.toThrow(
      '- IE_USD_1001.csv: unsupported CSV header',
    );
    await expect(new CsvStatementSource(folder).load()).rejects.toThrow(
      '- statement.csv: unsupported filename',
    );
  });

  it('proceeds when the folder contains only supported CSV files', async () => {
    const folder = await mkdtemp(join(tmpdir(), 'budget-audit-'));
    await writeFile(
      join(folder, 'IE_USD_1001.csv'),
      `${header}\n15/05/2026,Transfer,001,ACC,10.00,0.00,"4,000.00",0.00,Employer,Salary,Incoming\n`,
    ...
    await expect(new CsvStatementSource(folder).load()).resolves.toMatchObject({
      transactions: [expect.objectContaining({ transactionNumber: '001' })],
    });
  });
});
